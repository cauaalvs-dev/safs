...
// Sanitiza antes de validar. Remove chars que quebram URIs mailto: ou injetam markup
// se esses valores forem refletidos em outro contexto no futuro.
...
// Intencionalmente mais restritiva que RFC 5322 — evita surpresas com unicode
// e caracteres especiais antes de repassar ao cliente de e-mail.
...
const inputStyle: React.CSSProperties = {
  ...
  transition: "border-color 0.2s", background: "white", color: "#1a2e35",
};
...
export default function Contato() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const nome     = sanitize(form.nome,     200);
    const email    = sanitize(form.email,    200);
    const mensagem = sanitize(form.mensagem, 1000);

    if (nome.length < 2)      { setErro("Informe seu nome."); return; }
    if (!isValidEmail(email)) { setErro("Informe um e-mail válido."); return; }
    if (mensagem.length < 10) { setErro("Mensagem muito curta. Mínimo 10 caracteres."); return; }
          ...
          <div style={{ background: "white", borderRadius: 24, padding: 44, boxShadow: "0 4px 28px rgba(0,0,0,0.07)" }}>
            ...
            {enviado ? (
              ...
              <form onSubmit={handleSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: 22 }}>
                {erro && (
                  <div role="alert" aria-live="assertive" style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", borderRadius: 10, padding: "12px 16px", fontSize: 14, fontWeight: 600 }}>
                    {erro}
                  </div>
                )}
                ...
                <div>
                  ...
                  <input
                    id="nome" type="text" required autoComplete="name" spellCheck={false}
                ...
                <div>
                  ...
                  <input
                    id="email" type="email" required autoComplete="email" spellCheck={false}
                ...
                <div>
                  ...
                  <textarea
                    id="mensagem" rows={5} required spellCheck={false}
                    placeholder="Como podemos ajudar?" value={form.mensagem}
