...
const projetos: Projeto[] = [
  ...
  },
  {
    ...
    atividades: [
      ...
      "Palestras e oficinas educativas com apoio municipal",
...
function ProjetoCard({ projeto }: { projeto: Projeto }) {
  return (
    <div className="card-projeto">
      <div style={{ padding: "28px 32px 24px", background: projeto.corBg }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 18 }}>
          <div style={{ width: 68, height: 68, borderRadius: 16, background: "white", boxShadow: "0 2px 10px rgba(0,0,0,0.08)", flexShrink: 0, position: "relative", overflow: "hidden" }}>
            <Image src={projeto.logo} alt={projeto.logoAlt} fill style={{ objectFit: "contain", padding: 8 }} />
          </div>
          <div>
            <h3 style={{ fontWeight: 900, fontSize: "1.05rem", color: "#1a2e35", marginBottom: 6, lineHeight: 1.3 }}>{projeto.titulo}</h3>
            <p style={{ fontSize: 13, fontWeight: 700, color: projeto.cor, lineHeight: 1.4, margin: 0 }}>{projeto.subtitulo}</p>
          </div>
        </div>
      </div>

      <div style={{ padding: "24px 32px 32px" }}>
        <p style={{ color: "#555f63", fontSize: 14, lineHeight: 1.8, marginBottom: 20 }}>{projeto.descricao}</p>
        ...
        <ul style={{ listStyle: "none", padding: 0, display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
          {projeto.atividades.map((atividade) => (
            <li key={atividade} style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 14, color: "#555f63" }}>
              <span style={{ width: 18, height: 18, borderRadius: "50%", background: projeto.cor, flexShrink: 0, marginTop: 2, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ background: projeto.corBg, borderRadius: 10, padding: "10px 16px", fontSize: 13, fontWeight: 700, color: projeto.cor }}>
          {projeto.detalhe}
        </div>
      </div>
    </div>
